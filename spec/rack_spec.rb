...
describe "Rack::Router" do
  ...
  it "updates PATH_INFO and SCRIPT_NAME correctly in child routers" do
    prepare do |r|
      r.map "/hello", :to => make_router_and_prepare { |c| c.map "/world", :to => HelloApp }
    end
  ...
  it "does not let updated PATH_INFO and SCRIPT_NAME bleed across routes" do
    prepare do |r|
      r.map "/hello", :to => make_router_and_prepare { |c| c.map "/world", :to => WorldApp }
      r.map "/hello", :to => make_router_and_prepare { |c| c.map "/america", :to => AmericaApp }
    end
